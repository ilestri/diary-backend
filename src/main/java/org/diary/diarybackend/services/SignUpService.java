...
import org.diary.diarybackend.controllers.dtos.SignUpResDTO;
    ...
    public SignUpResDTO signUp(SignUpReqDto signUpReqDto, BindingResult bindingResult) {
        ...
        try {
            ...
            User user = User.builder().username(signUpReqDto.getUsername())
                    .phoneNumber(signUpReqDto.getPhone_number()).email(signUpReqDto.getEmail())
                    .password(encodedPassword).birthdate(signUpReqDto.getBirthdate()).build();

            User savedUser = usersRepository.save(user);

            User_Profile userProfile = User_Profile.builder().user(savedUser)
                    .nickname(signUpReqDto.getNickname()).build();

            userProfileRepository.save(userProfile);

            log.info("회원가입 성공: 이메일 = {}", signUpReqDto.getEmail());
            return new SignUpResDTO(200, "회원가입 성공", "SUCCESS");

        } catch (Exception e) {
            log.error("회원가입 중 예상치 못한 오류 발생: {}", e.getMessage());
            return new SignUpResDTO(500, "회원가입 중 예상치 못한 오류가 발생했습니다.", "FAILURE");
        }
    }
}
